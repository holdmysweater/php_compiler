...
#define CONSOLE_COUT(expr) do {} while (0)
#define CONSOLE_LOG(expr)  do {} while (0)
#else
#define CONSOLE_COUT(expr) std::cout << expr
#define CONSOLE_LOG(expr)  expr
#endif
...
#else
    Log(Color::Bold() + Color::Grey() + "% CONSOLE_NODE_LOG_DISABLED %");
#endif
...
#else
    Log(Color::Bold() + Color::Grey() + "% CONSOLE_PARSER_LOG_DISABLED %");
#endif
...
#ifdef CONSOLE_LEXER_LOG_ENABLED
    CONSOLE_COUT(
        Color::Yellow() << "L = " << line << Color::Grey() << " \t'"
        << (isSecondaryColor ? Color::Cyan() : Color::Green())
...
#ifdef CONSOLE_LEXER_LOG_ENABLED
    CONSOLE_COUT(
        Color::Yellow() << "L = " << lineStart << "-" << lineEnd
...
#ifdef CONSOLE_NODE_LOG_ENABLED
    Log(Color::Grey() + "%NODE% " + std::to_string(id) +
        " (" + type + "): " + message);
#endif
}
